# Train/Fine Tune SAM 2 on UFO120 dataset
# This mode use several images in a single batch
# UFO120 dataset contains underwater object segmentation masks
...
# Read data
data_dir = "UFO120/"  # Path to UFO120 dataset
train_img_dir = os.path.join(data_dir, "train/lrd")  # LRD images
train_mask_dir = os.path.join(data_dir, "train/mask")  # Mask images
...
def load_dataset(img_dir, mask_dir, val_split=0.2):
    ...
    for img_path in image_files:
        ...
        # Try multiple ways to find the matching mask
        possible_mask_names = [
            # Same filename with same extension
            os.path.join(mask_dir, base_name),
            # Same filename with .png extension
            os.path.join(mask_dir, os.path.splitext(base_name)[0] + '.png'),
            # Same filename with .jpg extension
            os.path.join(mask_dir, os.path.splitext(base_name)[0] + '.jpg')
        ]
        
        mask_path = None
        for path in possible_mask_names:
            if os.path.exists(path):
                mask_path = path
                break
        
        if mask_path:  # Only include images that have corresponding masks
            data.append({
                "image": img_path,
                "mask": mask_path,
            })
